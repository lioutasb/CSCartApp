...
public class CustomerOrderParser {
    public CustomerOrderItem parse(JSONObject orderJ){
        ...
        try {
            item.setOrder_id(orderJ.getString("order_id"));
            item.setCurrency(orderJ.getString("currency"));
            item.setDate_added(orderJ.getString("date_added"));
            item.setDisplay_id(orderJ.getString("display_id"));
            item.setEmail(orderJ.getString("email"));
            item.setFirstname(orderJ.getString("firstname"));
            item.setLastname(orderJ.getString("lastname"));
            item.setBarcode_img_url(orderJ.getString("barcode_img_url"));
            item.setInvoice_pdf_url(orderJ.getString("invoice_pdf_url"));

            CustomerOrderStatusItem statusItem = new CustomerOrderStatusItem();
            statusItem.setComments(orderJ.getJSONObject("order_status").getString("comments"));
            statusItem.setPosition(orderJ.getJSONObject("order_status").getInt("position"));
            statusItem.setStatus_id(orderJ.getJSONObject("order_status").getString("status_id"));
            statusItem.setStatus_name(orderJ.getJSONObject("order_status").getString("status_name"));
            statusItem.setStatus_type(orderJ.getJSONObject("order_status").getString("status_type"));
            statusItem.setStatus_color(orderJ.getJSONObject("order_status").getString("status_color"));
            ...
            JSONArray products = orderJ.getJSONArray("order_items");
            for(int i = 0; i < products.length(); i++){
                CustomerOrderProductItem product = new CustomerOrderProductItem();

                product.setPrice(products.getJSONObject(i).getString("price"));
                product.setThumbnail_pic_url(products.getJSONObject(i).getString("thumbnail_pic_url"));
                product.setDisplay_attributes(products.getJSONObject(i).getString("display_attributes"));
                product.setFinal_price(products.getJSONObject(i).getString("final_price"));
                ...
                product.setOrder_item_id(products.getJSONObject(i).getString("order_item_id"));
                product.setQty(products.getJSONObject(i).getString("qty"));
                product.setVirtual_flag(products.getJSONObject(i).getString("virtual_flag"));
            ...
            shippingAddress.setAddress_type(shipping_address.getString("address_type"));
            ...
            shippingAddress.setCountry_name(shipping_address.getString("country_name"));
            ...
            billingAddress.setCountry_code(billing_address.getString("country_code"));
            billingAddress.setCountry_id(billing_address.getString("country_id"));
            billingAddress.setCountry_name(billing_address.getString("country_name"));
            ...
            JSONObject shipping_method = orderJ.getJSONObject("shipping_method");
